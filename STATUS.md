...
| **ai-agents-for-work** — AI Agents for Your Job | 76 (12 sections) | `85 fail · 0 warn` — same two gate types | Full 12-section editorial pass, 0 warnings, screenshot inventory complete. Full report: [`courses/ai-agents-for-work/EDITORIAL-REPORT.md`](courses/ai-agents-for-work/EDITORIAL-REPORT.md) | Read and `verified: true` on all 76 lectures; fill `[INSTRUCTOR-INPUT]` on ~2 dozen `voice: human` lectures (0.1, 0.2, section intros/recaps, 6.0, 8.6, 11.2/11.3/11.5); capture 2 screenshots once the instructor's own email/Google accounts exist; add Ravi Chandra to `story-bible.yaml` cast list |
