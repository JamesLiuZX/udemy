...
This is worth checking against chapter eleven's decision tree directly, because it revises one of that tree's own answers. "Needs facts that change" pointed toward retrieval. The fuller version of that same test is needs facts that change slower than you can realistically re-index. Past that speed, the honest fix isn't a better pipeline. It's a live tool call at the moment of the question, the agent shape from chapter seven, checking the actual current system instead of a snapshot of it that's already a little bit wrong the moment it's built.
