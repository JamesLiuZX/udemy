...
[KEY-INSIGHT: 2024年的两项独立研究，都在公开的大语言模型基准测试中发现了大范围的数据污染。一份开源数据污染报告估计，MMLU（当时被引用最多的公开知识类基准测试之一）中有29.1%的测试项目，已经泄漏进模型的训练数据。一项由耶鲁大学（Yale）研究者主导、发表于NAACL的研究，则用另一条路径得出了同样的结论：把基准题目中的一个答案选项遮住，让模型逐字复原缺失的那个选项，ChatGPT和GPT-4分别在52%和57%的情况下做到了，远远高于单靠题目本身推理应有的水平。 || 来源（Source）：Yucheng Li et al., "An Open-Source Data Contamination Report for Large Language Models," Findings of EMNLP 2024；Chunyuan Deng et al., "Investigating Data Contamination in Modern Benchmarks for Large Language Models," Proceedings of NAACL 2024。]

一个在自己可能已经部分背下答案的基准测试上表现良好的模型，并不是在对你撒谎，确切地说。它只是在回答一个和头条百分比暗示的那个问题略有不同的问题。行业对这一发现的回应，也值得了解：被污染的基准测试，会以一到两年为周期被淘汰、换成更新更难的版本，这意味着任何一份厂商演示材料里出现的具体基准名称，都会一直变下去。而"这套测试集里到底装了什么"这个提问习惯，才是能一直带着走的部分。两个习惯，能捕捉到这里大多数真正要紧的东西。第一，检查这个基准测试里是否包含任何和你真实任务类型相似的内容：大多数通用基准测试，都严重依赖数学、代码和常识题，因为这类题目最便宜、最容易自动打分，而一个负责总结客服工单、或者起草营销文案的功能，可能和真正被测试过的东西没有任何共同点。第二，把一个公开基准测试当作一道底线，而不是一道天花板：一个整体表现不佳的模型，大概率确实是能力不足，但一个表现良好的模型，只能告诉你它大概没有明显的问题，不能告诉你它在你这个具体任务上是否真的好用。第四章的评测集，才是真正回答第二个问题的东西，没有任何基准测试能替代它。
