...
[PULLQUOTE: 不是在每一个功能上把每一章的清单都全套走一遍：识别出对眼前这种形态而言，哪两三个问题携带着真正的风险，把你有限的时间花在那里。]

三个结论并排放在一起，就是整套练习的压缩版：

| | 发票抽取 | 线索评分 | 编程智能体 |
| --- | --- | --- | --- |
| 形态 | 抽取器 | 预测器 | 智能体 |
| 真正要紧的风险 | 悄悄填错的字段 | 学来的偏见、漂移 | 复合效应、影响半径 |
| 承载它的章节 | 第四、八章 | 第四、八、九章 | 第七、八章 |
| 上线时附带的边界 | 没有具名审批人就不付款 | 只展示信号，绝不做隐藏过滤器 | 不合并、不部署、测试把关 |
