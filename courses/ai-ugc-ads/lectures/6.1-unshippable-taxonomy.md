...
@slide statement
kicker: Section 6 · The taxonomy
## Every AI ad that gets pulled, flagged or refunded fails for one of four reasons. Almost never a fifth.
lead: Name the four, and a pre-flight check stops being a vague worry and becomes a specific thing to look for.

@narrate
You've already met every one of these failures. They just weren't named
together until now, because each section taught you the fix for one of
them at the exact moment you needed it.

Four categories cover almost everything that actually gets an ad pulled,
...
into one of these four. Very few sit outside them.

Why does naming them matter, rather than just remembering the individual
fixes? Because a name is what lets you check for a failure you haven't
personally seen yet. You already know what a mangled spec overlay looks
like, because Section 3.4 showed you one. You may never have seen a
music-licensing gap specifically. But if you know "rights gap" is a
category worth checking, you'll catch it the first time it shows up in
your own batch, not the second time, after it's already cost you
something.
...
```text
1. CLAIMS      Could I defend this exact sentence with a study
               or a spec, out loud, to a compliance officer?
2. RIGHTS      Is every voice, face and piece of music in this
               licensed for exactly this use?
3. ACCURACY    Would the actual founder recognise this as the
               real product, held for the full shot?
4. DISCLOSURE  Does this need an AI label on the platform
               it's about to run on, and does it have one?
```
...
Ask yourself which of these four you'd be most tempted to skip on a
Friday afternoon, batch due, budget already approved. Be honest with
yourself about the answer, because that's exactly the question worth
building a habit around rather than trusting willpower to catch every
time.
...
Keep the four-question framework from this lecture close at hand while
you go. Each of the next three lectures takes one question and turns it
into a real, specific check. By Section 6.6, all four come back together
as one checklist, run once on a whole batch instead of once per
question.
...
already know, roughly, where your own set is strong and where it's
worth paying closer attention as those lectures arrive.
