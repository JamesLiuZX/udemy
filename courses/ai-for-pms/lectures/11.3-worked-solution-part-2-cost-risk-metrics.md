...
## The same refund agent, the other four artifacts: what it costs, which model earns its place, what could go wrong, and how you'll know it's working.
lead: Where 11.2 asked whether the feature works, this lecture asks whether it's worth building, safe to ship, and honestly measured after launch.
...
built. The cost model needs the tool-call sequence from Section seven.
The model scorecard needs the golden set's zero-tolerance bar to even
know what it's scoring against. That dependency isn't a coincidence.
It's exactly the internal consistency 11.1 asked every capstone package
to have.
...
much recovered time is not a close call.
...
Notice, too, what this table leaves out entirely: engineering time to
build and maintain the integration, the tool calls into the order
system, the escalation queue a human still has to work. A cost model
that only counts the model's own API bill is answering a narrower
question than "what does this feature cost," and saying so plainly is
better than letting a small, honest number imply a completeness it
doesn't have.
...
This table deliberately drops 7.4's p95 latency column, not because
latency stopped mattering, but because 7.6 already settled it for this
feature: the three tool calls, not the model's own thinking time,
dominate the wait either way. Swapping Flagship for Mid-tier barely
moves the number a customer actually feels, which is one more reason
the safety gate and the cost column are doing all the real work in
...
just the one number this lecture has been quoting in aggregate.

The last row deserves the most attention of the five, because it's the
...
a direct replacement. Acceptance measures how often a human keeps a
suggestion, which assumes a human is in the loop deciding. This
feature's auto-approve path has no such human, so the honest
adaptation is the production version of 11.2's own zero-tolerance bar:
wrongful-approval rate, watched continuously, the same discipline 8.6
asked of exactly this kind of quiet drift.
...
1. Check your own model choice against a safety gate, not cost first
2. Label your own cost model's numbers: measured, or still projected
...
model ever won by skipping that order, revisit the decision.

Then label your own cost model honestly, the same exercise this
