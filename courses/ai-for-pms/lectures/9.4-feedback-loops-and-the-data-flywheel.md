...
loop stops exactly there, quietly, while the usage number on the
dashboard keeps climbing regardless.
...
automated version this lecture is warning you away from, and it's
missing from the drawing on purpose, not by oversight. Every real
version of this loop routes through a person at the review step, every
time, with no exception carved out for convenience.

Compare this loop against 4.2's golden set build process, because it's
the same shape stretched out over time instead of done once. A golden
set was fifty cases, chosen deliberately, scored against a rubric, at a
single moment before launch. This loop is the same deliberate selection
and review, running in small batches, forever, after launch.
