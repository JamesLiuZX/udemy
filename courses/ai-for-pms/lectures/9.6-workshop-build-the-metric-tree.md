...
actually recognise on sight. ==A tree with one honest gap is worth more
than one with five numbers nobody checked==, the same bar as every
workshop this course has run.
:::

@narrate
Open A12, and give this the same twenty minutes every workshop in this
...
| 8-12 | Your last A/B test against 9.3's checklist | Randomised by user, version pinned |
...
test's setup, not recall it from memory. Randomised by user, version
...
Choose the North Star carefully: it's doing more work than it looks
like. For the refund agent this course keeps returning to, deflection
is often the honest choice, the number that ties usage directly to the
cost Section seven taught you to compute. For a drafting tool,
acceptance might sit at the top instead, with adoption and a cost
number as its branches. Which one is honestly true of your feature?

Notice this tree is deliberately shallow, one level, not a sprawling
org chart. Depth is what the team's own dashboard from 9.5 is for. It
exists to be read in the time it takes to say four sentences out loud,
roughly how long you'll have in the room that matters.

The cost branch is easy to leave off a tree that otherwise feels
complete. Pull it directly from Section seven's unit economics model
rather than estimating it fresh. If 7.2's cost model and 7.5's margin
work are already built, this branch is a five-minute copy, the same
shortcut this workshop already gave you for adoption and acceptance.
...
Finish the tree with a deliberately chosen North Star, and send it to whoever owns your next leadership update
...
to be easiest to find. Then hand it to whoever actually presents this
feature next, and ask which branch they'd push back on first.
