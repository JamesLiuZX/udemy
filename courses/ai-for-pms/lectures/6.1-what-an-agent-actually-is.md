...
notes: >-
  ...
  actual mechanism. Sets up 6.2 (tools/MCP), 6.3 (reliability maths), and
...
Run the test on one "AI agent" product you use, and write down one concrete decision point where it could plausibly go wrong
...
the moment. Then name one point where that decision could go wrong,
concretely, not in the abstract. "It might misunderstand the customer"
is too vague to act on. "It might approve a refund for an item that was
already returned once" is the kind of finding that actually changes what
you build.
