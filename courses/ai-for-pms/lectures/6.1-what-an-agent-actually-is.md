...
Take a refund request landing in a support inbox. An agent observes the
email, decides it needs the order history, calls a tool to pull it,
observes what came back, and only then decides whether this qualifies for
an automatic refund or needs a human. That decision genuinely depends on
what the order history showed. Nobody wrote "always approve" or "always
escalate" into the system in advance. The model decided, in the moment,
...
A workflow is predictable precisely because nobody left room for it to
