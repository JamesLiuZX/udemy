...
A token is roughly three quarters of a word in English, but not evenly.
Common short words like "the" are one token. Longer or rarer words get
split: "summarisation" is three. Even a contraction like "don't" is two,
because the apostrophe splits it.

A typical support ticket reply runs forty to sixty tokens. That number
matters in a minute, because your context window has a fixed token
budget, and every part of your prompt, the system instructions, the
conversation, the documents you retrieved, is drawing from the same pool.
...
1. Find your feature's typical input length in tokens, not words
2. Test it once at three times your longest expected input

@narrate
Two things before you move on.

Find out, roughly, how many tokens your feature typically handles, using
real examples, not the three-message demo. Then stretch one real case
until it's three times longer than anything you've actually seen, and
watch what happens to the details near the start.
