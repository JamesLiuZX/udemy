...
Count the handoffs in a multi-agent proposal, and ask whether it's been tried locally yet or is heading straight to production

@narrate
One thing before you move on.

Count the handoffs specifically, the same way you counted steps for
6.3's table, and ask what a misread at each one actually costs. Then ask
the deployment question directly: "prototyping locally first" and
"building straight into the production pipeline" are two different risk
profiles wearing the same architecture diagram.
