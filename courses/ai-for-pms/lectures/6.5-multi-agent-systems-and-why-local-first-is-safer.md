...
verified: false
objectives:
  - "Explain why multi-agent orchestration adds handoff points that 6.3's reliability math applies to as well"
  - "State the concrete case for prototyping multi-agent work locally before deploying it hosted"
notes: >-
  Extends 6.3's reliability math to the handoffs between agents, and
...
## One agent's reliability math was already sobering. A second agent doesn't fix that, it adds a handoff.
...
Count the actual chain length honestly and it's longer than the diagram
first suggests. The planner's own decisions, each worker's internal
...
Take a multi-agent proposal you've seen and ask what a single, better-
scoped agent would fail to do. If the honest answer is "nothing
