...
Nobody gets promoted for the AI feature they didn't ship. That's exactly
why this checklist matters, because the pressure in every room is toward
building something, and the discipline you need is the same one from the
rest of this course, pointed at the decision itself instead of at the
output.
...
| --- | --- |
| Is a wrong answer severe and irreversible, with no human check first? | Stop until a human checkpoint exists |
| Does this need to be correct every time, not just usually right? | A probabilistic system is the wrong tool |
| Can you neither ground it in facts nor verify it afterward? | You're flying on hope, not evidence |
| Is the volume too low to justify a golden set at all? | The investment doesn't pay for itself yet |
| Would a simple rule already solve this, reliably, today? | You're adding risk to something that already works |
...
Read them again. Which one asks whether the model is capable enough? Not
one. Every question is about your situation: what happens when it's
wrong, whether you can check it, whether the volume justifies the
investment. That's deliberate. Capability keeps moving. Your situation is
