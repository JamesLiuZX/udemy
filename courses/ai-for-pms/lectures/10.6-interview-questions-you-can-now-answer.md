...
You're not in that position anymore, and this lecture's only job is
...
| --- | --- |
| How would you evaluate this feature | 4.2's golden set, 4.5's pass rate |
| How would you decide whether to ship it | 7.4's scorecard |
| What would worry you about this | 6.6's blast radius, 8.1's register |
| How would you know it's working after launch | 9.1's three-metric floor |

@narrate
Notice these four questions aren't a random assortment, they're the job
itself, asked in order: evaluate it, decide on it, de-risk it, watch it.
An interviewer asking all four isn't testing four unrelated skills.
...
Take the first row specifically, since it's the one almost every
candidate gets asked and almost none answer well. "I'd look at
accuracy" isn't wrong, it's just not an answer yet. "A fifty-case golden
set, weighted toward the boundary cases, scored against a rubric two
people agree on" is the same idea, except now it's something you've
actually built and can describe in the order you built it.

The second row catches a confusion worth clearing up before anyone
asks it directly: passing an eval and being ready to ship are two
different questions, and answering as though they're the same one is a
fast way to sound junior in the room. 7.4's scorecard exists precisely
because the ship decision weighs the pass rate against cost, latency,
and how bad the failure mode actually is, not the pass rate by itself.
Naming those variables unprompted is what separates someone who ran an
eval once from someone who's made a real ship call.

The third row is where most candidates go softest, because naming a
real worry out loud feels like admitting a weakness rather than
demonstrating one. It's the opposite. "I'd want to know the blast
radius before I got excited, here's what that means for this specific
feature" is a stronger answer than pretending no feature has ever
worried you, because the second version isn't true of anyone who's
actually shipped something.

The fourth row is the one candidates skip entirely most often, because
most interview prep stops at launch. Answering it well, adoption,
acceptance, deflection, the same three numbers 9.1 built a whole
lecture around, signals something specific: that you think about a
feature as a thing that keeps needing attention after the demo, not a
thing that's finished the day it ships.
