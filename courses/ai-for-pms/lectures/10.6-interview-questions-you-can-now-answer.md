...
attrib: What almost every candidate says, in spirit
...
@slide example
kicker: Question one, specifically
::: example "How would you evaluate this feature"
The one almost every candidate gets asked, and almost none answer
well. "I'd look at accuracy" isn't wrong, it's just not an answer yet.
:::

@narrate
"A fifty-case golden set, weighted toward the boundary cases, scored
against a rubric two people agree on" is the same idea, drawn from
4.2's golden set and 4.5's pass rate, except now it's something you've
...
@slide example
kicker: Questions three and four
::: example What worry, and what after launch
"I'd want to know the blast radius before I got excited" (6.6, 8.1).
Adoption, acceptance, deflection (9.1) once it's live, not just a
launch-day snapshot.
:::
...
weakness rather than demonstrating one. Isn't it actually the opposite?
Pretending no feature has ever worried you isn't true of anyone who's
actually shipped something, so naming the specific worry is the
stronger answer, not the riskier one.
...
@slide example
kicker: One more tell
::: example Order is its own signal
Build the set, agree the rubric, then measure. A candidate who lists
the same three ingredients out of order is usually reciting a
checklist, not describing something they've actually done.
:::

@narrate
Notice, too, that the good column answers in the order the work
actually happens, and that sequencing is a signal on its own, separate
from the content: rubric before cases, or measuring before agreement,
reads as memorised rather than lived.
...
Pick one question, rehearse it live against an unfamiliar feature, and revisit whichever lecture your weakest answer points back to

@narrate
One thing before you move on.

Pick one question, not all four at once, and actually say the answer out
loud to another person. Reading this lecture's table silently doesn't
build the same reflex as hearing yourself produce the answer under a
little real pressure. Then be honest about which question stalled you,
and go back to that specific lecture. The fix for a weak answer on blast
radius is 6.6 and 8.1 again, not another pass through this summary.
