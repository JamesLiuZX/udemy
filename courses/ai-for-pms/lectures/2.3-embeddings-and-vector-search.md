...
## "Vector search" sounds like it needs a maths degree to discuss. It needs one idea: turn meaning into a location, then find what's nearby.
lead: No linear algebra in this lecture. Just what's actually happening between your question and the paragraph it retrieves.

@narrate
Last lecture put RAG on the decision tree without opening it up. Here's
what's actually inside that box, because "it searches a vector
database" is not an explanation, it's a phrase people repeat to sound
...
You don't need to know how those numbers get produced, the same way
you don't need to know how a search engine ranks pages to use one well.
What you need is the one consequence that actually changes how you use
this: the search is entirely about meaning, never about the specific
words typed. Ask about "cancelling a subscription" and it can surface a
paragraph that says "ending your membership" without either sentence
sharing a single word. That's the whole point of doing it this way
instead of the old keyword search it replaced.
...
Ask, on any RAG feature you touch, who owns re-indexing when a source
