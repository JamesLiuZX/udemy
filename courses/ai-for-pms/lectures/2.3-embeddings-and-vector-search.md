---
id: "2.3"
title: "Embeddings and vector search without the maths"
section: 2
duration_target: 7
voice: tts
verified: false
...
@narrate
You'll never build this pipeline yourself, so treat what follows as the
machinery behind one decision that actually is yours: who owns
re-indexing when a source document changes, which the honest caveat at
the end of this lecture comes back to.
...
Pull one real retrieved result from your own feature, read it properly, and check whether the chunk boundaries respect the document's structure
