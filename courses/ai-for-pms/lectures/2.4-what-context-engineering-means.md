...
What does a real feature actually send the model? Far more than your
instruction. It sends retrieved documents, the last several messages of a conversation,
...
1. Find one real wrong answer and read its full sent context, not just the prompt
2. Name which slot the actual problem was sitting in
...
real one. Read every slot, in order.

Then name which slot the problem actually lived in. Instructions,
examples, retrieval, history, tools, or truly the wording. Most of the
time it won't be the last one.
